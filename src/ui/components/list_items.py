...
def SearchResultItem(page: ft.Page, item_data, item_type: str, on_download=None):
    ...
    if item_type == "artist":
        ...
        subtitle = item_data.artist.name
...
def AlbumCard(page: ft.Page, album_data):
    ...
    return ft.Container(
        content=ft.Column([
            ft.Image(
                src=album_data.cover_medium,
                border_radius=ft.border_radius.all(8),
                width=150,
                height=150
            ),
            ft.Text(album_data.title, size=14, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER),
            ft.Text(album_data.release_date.split('-')[0], size=12, color=theme.SECONDARY_TEXT, text_align=ft.TextAlign.CENTER),
        ],
